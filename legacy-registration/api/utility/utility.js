...
exports.decrypt = function (object, secret, salt) {
    ...
    if (salt) {
        // crypto function call to get key for decryption

        let key = crypto.pbkdf2Sync(secret, salt, 1000, 64, 'sha1');

        // Convert crypto key to hexa data type.
        key = key.toString('hex');

        // Encode key
        key = stablelibutf8.encode(key.substring(0, nacl.secretbox.keyLength));

        // Decrypt object with derived key
        let decrypted;

        try {
            decrypted = exports.decryptObject(object, key);
            r.resolve(decrypted);
...
exports.decryptCredentials = function (object, secret, salt) {
    ...
    if (salt) {
        ...
        let key = crypto.pbkdf2Sync(secret, salt, 1000, 64, 'sha512');
